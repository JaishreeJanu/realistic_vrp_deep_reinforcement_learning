...
class AttentionModel(nn.Module):
    ...
    def forward(self, batch):
        ...
        if self.checkpoint_encoder and self.training:  # Only checkpoint if we need gradients
            embeddings, _ = checkpoint(self.embedder, self.init_embed(batch_obs["node_features"]))
        else:
            node_embeddings = self.init_embed(batch_obs["node_features"])
            #print(node_embeddings.shape)
            embeddings, _ = self.embedder(node_embeddings)
...
#
    ...
    def _get_parallel_step_context(self, embeddings, batch_obs, from_depot=False):
        ...
        current_node = torch.Tensor(batch_obs['curr_pos_idx']).type(torch.int64) #batch_obs.get_current_node()
        batch_size = batch_obs['ids'].size
        num_steps = 1
        ...
        return torch.cat((node_context,
                torch.Tensor(batch_obs['remaining_capacity']).view(-1, 1)),
                -1)
